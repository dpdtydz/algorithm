...
# 걸어서 가장 짧은 거리에 있는 방을 선호한다고 한다.
#
# 여러분은 지우를 도와 줄 프로그램을 작성하고자 한다. 즉 설문조사 결과 대로 호텔 정문으로부터 걷는 거리가 가장 짧도록 방을
# 배정하는 프로그램을 작성하고자 한다.
#
...
#
# 이런 형태의 호텔을 H × W 형태 호텔이라고 부른다. 호텔 정문은 일층 엘리베이터 바로 앞에 있는데, 정문에서 엘리베이터까지의
# 거리는 무시한다. 또 모든 인접한 두 방 사이의 거리는 같은 거리(거리 1)라고 가정하고 호텔의 정면 쪽에만 방이 있다고 가정한다.
...
for i in range(n):
    ...
    a = n % h  # 만약 304라면 a= 3
    b = n // h + 1  # 만약 304라면 b=4
    if a == 0:  # a가 0일경우
        a = h  # 호텔의 꼭대기층
        b -= 1 # 그럼 a의 값이 0 이되니 a가 0일떄는 h의 값을 a에담고 b에는 -1을 해준다.
    print(a*100+b) # 층단위인 100을 곱해주고 앞자리와 뒷자리를 붙여준다
